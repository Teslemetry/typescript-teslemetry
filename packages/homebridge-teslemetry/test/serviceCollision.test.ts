...
test("Door(x6), RearDefrost, and TPMS(x5) each get their own Service.ContactSensor instance", () => {
	const { accessory } = setup();
	const contactSensors = accessory.services.filter((s) => s.UUID === Service.ContactSensor.UUID);
	assert.equal(contactSensors.length, 12);
	assert.ok(contactSensors.some((s) => s.subtype === "rear-defrost"));
	assert.ok(contactSensors.some((s) => s.subtype === "tpms-hard"));
});
