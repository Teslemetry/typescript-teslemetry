...
// Configuration
const API_KEY = process.env.GEMINI_API_KEY || process.env.GOOGLE_API_KEY || "AIzaSyDiKx0p57I2yH0GpiTH3xrJrwPRg2EN5gY";
if (!API_KEY) {
...
const DRIVER_WALL_CONNECTOR_DIR = path.resolve("drivers/wall-connector/assets/images");

const ROOT_ASSETS_ICON = path.resolve("assets/icon.svg");
const BRANDMARK_PATH = path.resolve("assets/Brandmark - ELECTRIC TIGHT.png");

const dirs = [ASSETS_DIR, DRIVER_VEHICLE_DIR, DRIVER_POWERWALL_DIR, DRIVER_WALL_CONNECTOR_DIR];
...
const tasks: ImageTask[] = [
  ...
  // Resolutions: Small 250x175, Large 500x350, XLarge 1000x700 (Aspect ~1.43:1)
  {
    ...
    prompt: "A bright, modern, and clean smart home energy scene. A white electric vehicle charging in a minimalist garage with a home battery unit on the wall. Soft daylight lighting, photorealistic, 3D render style. No text, no logos, no people. Visualizes energy flow and connectivity.",
  ...
  // Resolutions: Small 75x75, Large 500x500, XLarge 1000x1000 (Square 1:1)
  {
    ...
    prompt: "A studio shot of a white Tesla Model 3 from a front 3/4 angle. Pure white background, soft studio lighting, sharp details, photorealistic. The car is isolated on white. No text.",
  ...
  },
  {
    ...
    prompt: "A studio shot of a Tesla Powerwall 2 battery unit. Pure white background. Front view, clean, minimalist, photorealistic. The unit is isolated on white. No text.",
  ...
  },
  {
    ...
    prompt: "A studio shot of a Tesla Wall Connector Gen 3. Pure white background. Front view showing the white glass faceplate and the charging cable coiled neatly. Photorealistic. Isolated on white. No text.",
  ...
  for (const driverDir of driverAssetsDirs) {
      ...
      try {
          ...
          try {
             ...
             console.log(`[WARN] Copied placeholder icon.svg to ${dest}. GUIDELINE CHECK: Replace this with a unique device SVG.`);
          } catch (e) { /* ignore */ }
      }
  }
  ...
  for (const task of tasks) {
    ...
    try {
      ...
      try {
          ...
          if (!imageBuffer && data.error) console.error("  - Gemini Error:", data.error.message);
      } catch (e) {
           ...
           try {
               ...
               const resp = await fetch(imagenUrl, {
                 ...
                 body: JSON.stringify({
                    ...
                    parameters: { sampleCount: 1, aspectRatio: task.sizes.some(s=>s.width > s.height) ? "16:9" : "1:1" }
                 })
               });
               const data = await resp.json();
