...
**Gotcha**: `DoorService`'s frunk/trunk contact sensors are registered unconditionally, present even on vehicles without a powered frunk - this is a known legacy gap, not a pattern to extend. A single-model signal like Cybertruck's `TonneauOpenPercent` (`vehicle-services/tonneau.ts`) instead gates its service in `vehicle.ts` on `useTeslaModel(vehicle.vin) === "Cybertruck"` (`@teslemetry/api`'s VIN-based model discriminator, `Teslemetry.ts`) before constructing it.
